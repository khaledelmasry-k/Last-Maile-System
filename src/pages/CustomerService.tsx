...
export const CustomerService = () => {
  ...
  const trackedShipment = useMemo(() => {
    const q = tracker.trim().toLowerCase();
    if (!q) return null;
    return shipments.find((s) => [s.trackingNumber, s.phone].some((v) => String(v).toLowerCase() === q)) || null;
  }, [shipments, tracker]);
  ...
  return (
    <div className="page-wrap text-gray-900 dark:text-white h-full flex flex-col transition-colors duration-200">
      <header className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold font-sans tracking-tight">{t('Customer Service')}</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1 font-mono text-xs md:text-sm uppercase">{t('Manage Exceptions & Tickets')}</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1 min-h-0">
        <div className="lg:col-span-2 bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl flex flex-col overflow-hidden shadow-sm">
          <div className="p-5 border-b border-gray-200 dark:border-[#2a2a2a] bg-gray-50 dark:bg-[#0a0a0a] flex justify-between items-center">
            ...
            <span className="bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-500 text-xs font-bold px-2 py-1 rounded-full border border-red-200 dark:border-red-500/30">{exceptions.length} {t('Issues')}</span>
          </div>
              ...
              <div key={shipment.id} className="bg-gray-50 dark:bg-[#1a1a1a] border border-red-200 dark:border-red-500/30 rounded-lg p-5 hover:border-red-400 dark:hover:border-red-500/60 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mt-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-300 italic">"{shipment.timeline[shipment.timeline.length - 1]?.note || t('No notes provided')}"</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl p-6 flex flex-col shadow-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="w-full bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333] p-4 rounded-lg">
              <h3 className="font-bold text-orange-600 dark:text-orange-500 mb-2">{t('Track Shipment')}</h3>
              <input value={tracker} onChange={(e) => setTracker(e.target.value)} placeholder="Tracking number or phone" className="w-full mb-2 bg-white dark:bg-[#111] border border-gray-300 dark:border-[#333] rounded px-3 py-2 text-sm" />
              {trackedShipment ? (
                <p className="text-xs text-green-600 dark:text-green-400">{trackedShipment.trackingNumber} — {trackedShipment.status}</p>
              ) : tracker ? (
                <p className="text-xs text-red-500">No shipment found.</p>
              ) : null}
            </div>
          ...
          {(ticketLog.length || smsLog.length) ? (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-[#333] text-xs space-y-1 text-gray-500">
              {[...ticketLog, ...smsLog].slice(0, 6).map((l, i) => <p key={i}>• {l}</p>)}
            </div>
          ) : null}
        </div>
      </div>
